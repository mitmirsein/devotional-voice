# 📖 Devotional Voice Plugin for Obsidian v0.5.2

음성, 텍스트, 또는 노트 전체를 입력으로 받아 **AI 기반 묵상글**을 생성하고, **TTS(Text-to-Speech)**로 낭독해주는 Obsidian 플러그인입니다. v0.5.2 업데이트로 더욱 정교해졌습니다.

## ✨ 주요 기능 (v0.5.2 업데이트)

- **🚫 TTS 지문 자동 제거 (New)**: TTS 재생 및 오디오 생성 시, 대본에 포함된 괄호 속 지문(배경음약 시작, 차분하게 등)을 자동으로 감지하여 제거합니다. 이제 기계적인 지문 낭독 없이 순수한 멘트만 감상할 수 있습니다.
- **🎙️ Google Gemini STT 지원**: Whisper 외에 Google Gemini를 음성 인식 엔진으로 사용할 수 있습니다. 한국어 인식 및 장문 처리에 최적화되어 있습니다.
- **⏳ TTS 진행률 표시**: TTS 오디오 생성 시 진행 바(Progress Bar)를 통해 대기 시간을 시각적으로 확인합니다.
